...
int DispCtrlRpc::MapJsonToBinary(JsonDataCommObj* pReq,int index)
{
	...
	switch(command)
	{
		...
		case EJSON_SYSMGR_RPC_DISP_GET_BKLT:return json_to_bin_backlight_get(pReq);
		case EJSON_SYSMGR_RPC_DISP_SET_BKLT:return json_to_bin_backlight_set(pReq);
	}
	return -1;
}
/* ------------------------------------------------------------------------- */
int DispCtrlRpc::MapBinaryToJson(JsonDataCommObj* pReq,int index)
{
	...
	switch(command)
	{
		...
		case EJSON_SYSMGR_RPC_DISP_GET_BKLT:return bin_to_json_backlight_get(pReq);
		case EJSON_SYSMGR_RPC_DISP_SET_BKLT:return bin_to_json_backlight_set(pReq);
	}
	return -1;
}
/* ------------------------------------------------------------------------- */
int DispCtrlRpc::ProcessWork(JsonDataCommObj* pReq,int index,ADJsonRpcMgrProducer* pObj)
{
	...
	switch(command)
	{
		...
		case EJSON_SYSMGR_RPC_DISP_GET_BKLT:return process_backlight_get(pReq,pDataCache);
		case EJSON_SYSMGR_RPC_DISP_SET_BKLT:return process_backlight_set(pReq,pDataCache);
	}
	return 0;
}
/* ------------------------------------------------------------------------- */
...
{
	SYSMGR_PRINT_PACKET* pPanelCmdObj=NULL;
	PREPARE_JSON_REQUEST(RPC_SRV_REQ,SYSMGR_PRINT_PACKET,RPC_SRV_ACT_READ,EJSON_SYSMGR_RPC_DISP_GET_BKLT);
	return 0;
}
int DispCtrlRpc::bin_to_json_backlight_get(JsonDataCommObj* pReq)
{
PREPARE_JSON_RESP_ENUM(RPC_SRV_REQ,SYSMGR_PRINT_PACKET,SYSMGR_RPC_DISP_BKLT_ARG,bklsts,SYSMGR_RPC_DISP_BKLT_ARG_TABL,SYSMGR_BKLT_STS_UNKNOWN);
	return 0;
}
int DispCtrlRpc::process_backlight_get(JsonDataCommObj* pReq,SYSMGR_CMN_DATA_CACHE *pData)
{
	bool sts;
	DisplayDevice *pDisp=(DisplayDevice *)pData->pDisplay;
	RPC_SRV_REQ *pPanelReq=NULL;
...
{
	SYSMGR_PRINT_PACKET* pPanelCmdObj=NULL;
	PREPARE_JSON_REQUEST(RPC_SRV_REQ,SYSMGR_PRINT_PACKET,RPC_SRV_ACT_WRITE,EJSON_SYSMGR_RPC_DISP_SET_BKLT);
	JSON_STRING_TO_ENUM(SYSMGR_RPC_DISP_BKLT_ARG,SYSMGR_RPC_DISP_BKLT_ARG_TABL,SYSMGR_BKLT_STS,SYSMGR_BKLT_STS_UNKNOWN,pPanelCmdObj->bklsts);
	return 0;
}
...
int DispCtrlRpc::process_backlight_set(JsonDataCommObj* pReq,SYSMGR_CMN_DATA_CACHE *pData)
{
	DisplayDevice *pDisp=(DisplayDevice *)pData->pDisplay;
	RPC_SRV_REQ *pPanelReq=NULL;
	pPanelReq=(RPC_SRV_REQ *)pReq->pDataObj;
