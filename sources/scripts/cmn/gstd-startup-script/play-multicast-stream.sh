...
RES=$($GSTD_CLIENT_BIN --tcp-address=$GSTD_IP --tcp-port=5000 create /pipelines p1 udpsrc port=$STREAM_PORT multicast-iface=$MULTICAST_IFACE multicast-group=$MULTICAST_IP auto-multicast=true $CAPS ! queue ! tsdemux ! queue ! decodebin ! queue ! videoconvert ! queue ! xvimagesink sync=false async=false)
