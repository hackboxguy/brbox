...
RES=$($GSTD_CLIENT_BIN --tcp-address=$GSTD_IP --tcp-port=5000 create /pipelines p1 v4l2src device=$VIDEO_DEV ! videoconvert ! rtpvrawpay mtu=65000 chunks-per-frame=1 ! queue ! udpsink host=$TARGET_IP buffer-size=100000000 port=$STREAM_PORT sync=false async=false)
